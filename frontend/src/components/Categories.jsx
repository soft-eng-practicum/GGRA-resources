...
function Categories({
  ...
  const [outerOpen, setOuterOpen] = useState(null)
  const [innerOpen, setInnerOpen] = useState(null)

  useEffect(() => {
    if (selectedProvider) {
      setOuterOpen(`item-${selectedProvider.catId}`)
      setInnerOpen(`subitem-${selectedProvider.id}`)
    }
  }, [selectedProvider])
        ...
        {categories.map((category) => (
          ...
          >
            ...
            <AccordionContent>
              ...
              >
                {providers
                  .filter((p) => p.catId === category.catId)
                  .map((provider) => (
                    <AccordionItem
                      key={provider.id}
                    ...
                    >
                      <AccordionTrigger
                        className="px-3 rounded-md hover:bg-gray-100"
                      ...
                      <AccordionContent className="pt-3 px-4 pb-0">
                        <ul>
                          ...
                          <br />
                          <li>
                            <h3 className="font-bold">City</h3> {provider.city}
                          </li>
                          <br />
                          <li>
