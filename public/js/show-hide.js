...
async function fetchJson() {
  try {
    ...
    if (!resProviders.ok)
      throw new Error(`HTTP error! Status: ${resProviders.status}`)
    data = await resProviders.json() // Assign fetched data

    const resCategories = await fetch('/ggra-categories.json')
    if (!resCategories.ok)
      throw new Error(`HTTP error! Status: ${resCategories.status}`)
    dataCategories = await resCategories.json() // Assign fetched data
    ...
    newCreateOrganizationAndDetailsDivs(
      document.getElementById('categories'),
      data,
    )

    document.querySelectorAll('.classOrganizationDetails').forEach((el) => {
...
// adds a div as child of element
function addOrganizationDetailsDivToElement(
  element,
  content,
  idName,
  className0,
...
function newAddCategoryDiv(catId) {
  ...
  for (let index = 0; index < dataCategories.length; index++) {
    if (dataCategories[index].catId == catId) {
      target = index
    }
  }
  ...
  const newDiv = addDivToElement(
    categories,
    '',
    'catId' + dataCategories[target].catId,
    'classCategory',
  )
  const buttonDiv = addDivToElement(
    newDiv,
    dataCategories[target].type,
    'name' + dataCategories[target].catId,
    'classCategoryName',
  )
  ...
  imageExpand.addEventListener('click', function () {
    showHideCategory('class' + this.id.substring(6, this.id.length))
  })
  buttonDiv.addEventListener('click', function () {
    showHideCategory('class' + this.id.substring(4, this.id.length))
  })
...
function newCreateOrganizationAndDetailsDivs(element, data) {
  ...
  for (let index = 0; index < data.length; index++) {
    ...
    divOrganization = addDivToElement1(
      elementCategory,
      data[index].name,
      'index' + index,
      'class' + elementCategoryID,
      'partners',
    )
    divOrganization.style.display = 'none'

    // add listener to Organization show its details when clicked
    divOrganization.addEventListener('click', function () {
      showOrganizationDetails(this.id)
    })
    ...
    divOrganizationDetails = addOrganizationDetailsDivToElement(
      masterDiv,
      data[index].name,
      'index' + index + 'Details',
...
function showOrganizationDetails(organizationDetailsID) {
  ...
  var organizationDetails = document.getElementById(
    organizationDetailsID + 'Details',
  )
...
function showCategories() {
  for (let el of document.querySelectorAll('.classOrganizationDetails'))
    el.style.display = 'none'
...
function showHideClass(elementClass) {
  ...
  if (
    ...
    for (let el of document.querySelectorAll('.' + elementClass))
...
function showHideCategory(elementClass) {
  ...
  if (
    ...
    for (let el of document.querySelectorAll('.' + elementClass))
      el.style.display = 'block'
    document.getElementById('button' + catId).src =
      'public/img/Larks/arrow-collapse-upB.png'
    myMap = map
    ...
    for (let el of document.querySelectorAll('.' + elementClass))
      el.style.display = 'none'
    document.getElementById('button' + catId).src =
      'public/img/Larks/arrow-collapse-downB.png'
    myMap = null
